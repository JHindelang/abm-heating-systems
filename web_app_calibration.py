...
calib = pd.read_excel(os.path.join(
    folder_input, 'adoption_rates.xlsx'), header=2)
calib = calib.iloc[:,:]
...
calib_groups = {
    ...
    'heat_pump': ['heat_pump_gw', 'heat_pump_air', 'heat_pump_vc', 'heat_pump_hc'],
...
# configure plot for calibration data
plot_calib_2 = [{
        'x': calib_date,
        'y': calib[group],
        'name': ' tar ' + group,
        'type': 'line',
        'marker': {'color': diff_ms[0].color_list[group_color[group]]},
        'line': {'dash': 'dot'}
...
app.layout = html.Div(children=[
    ...
    html.Div([
        ...
        html.Div(id='rmsd', className='three columns'),
        html.Div(
...
@app.callback(
    ...
    [Output(component_id='rates', component_property='figure'),
     ...
     Output(component_id='calibration', component_property='figure'),
     Output(component_id='rmsd', component_property='children')],
...
def update_value(input_data1, input_data2, input_data3, input_data4, input_data5):
    ...
    rates_calib_ms = {}
    rmsd_l = 0
    for calib_group in calib_groups:
        rates_sys = [0] * steps_comp
        for model in diff_ms:
            for sys in calib_groups[calib_group]:
                rates_sys = [
                    rates_sys[i] + model.data_coll.model_vars[sys][i]/n_mods
                    for i in range(steps_comp)]
        rates_calib_ms[calib_group] = rates_sys
        devi = [
            (calib.iloc[i + 1][calib_group] - rates_calib_ms[calib_group][
                (i + 1) * 12 - 1]) ** 2
            for i in range(min(steps_comp // 12, max_calib))
        ]
        if steps_comp // 12:
            rmsd_l = rmsd_l + sqrt(sum(devi) / (steps_comp // 12))
    ...
    plot_calib = [{
            'x': x,
            'y': rates_calib_ms[group],
            'type': 'line',
            'name': group,
            'marker': {'color': diff_ms[0].color_list[group_color[group]]}
    } for group in calib_groups]

    plot_calib = plot_calib + plot_calib_2

    rates_plot = {
                'data': plot_data,
                'layout': {
                    'title': 'Share of adopters rates'
                }
    }
    ages_plot = {
                'data': plot_ages,
    ...
    calib_plot = {
        'data': plot_calib,
        'layout': {
            'title': 'Share of adopters calibration'
        }
    }

    color_update = {'color': diff_ms[0].hhs_color}

    rmsd_text = 'RMSD: ' + str(rmsd)

    return rates_plot, ages_plot, dec_plot, calib_plot, rmsd_text
